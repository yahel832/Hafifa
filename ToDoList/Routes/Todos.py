...
def create(request: CreateTodo, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
...
def get_current_user_todos(db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
    return Todos.get_current_user_todos(db, current_user)

@todo_router.get("/{todo_id}", status_code=status.HTTP_200_OK)
def get_by_id(todo_id: int, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
    return Todos.get_by_id(todo_id, db)

@todo_router.put("/{todo_id}", status_code=status.HTTP_200_OK)
def update(todo_id: int, request: TodoUpdate, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
    return Todos.update(todo_id, request, db)

@todo_router.delete("/{todo_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete(todo_id: int, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
    return Todos.delete(todo_id, db)

@todo_router.patch("/complete/{todo_id}", status_code=status.HTTP_202_ACCEPTED)
def complete(todo_id: int, db: Session = Depends(get_db), current_user: TokenData = Depends(get_current_user)):
