...
def get_all(db: Session, limit: Optional[int] = None, completed: Optional[bool] = None, username: Optional[str] = None):
    todos = db.query(Todo)
...
def create(request: CreateTodo, db: Session, current_user: TokenData):
...
def get_current_user_todos(db: Session, current_user: TokenData):
